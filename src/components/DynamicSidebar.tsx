...
import { ROUTES, toCanonicalUrl, isValidInternalLink } from '@/config/routes';
...
const SECTION_META: Record<SectionKey, { icon: React.ReactNode; title: string; accent: string; hrefBase: string; moreLabel: string; moreHref: string }> = {
    jobs:      { icon: <Briefcase size={13} />, title: 'Govt Jobs',   accent: 'text-blue-700',    hrefBase: '/job',    moreLabel: 'Sabhi Jobs',    moreHref: ROUTES.govtJobs },
    fastTrack: { icon: <Zap size={13} />,       title: 'Fast Updates', accent: 'text-amber-600',   hrefBase: '/update', moreLabel: 'Sabhi Updates', moreHref: ROUTES.govtJobs },
    tests:     { icon: <FileText size={13} />,  title: 'Mock Tests',  accent: 'text-emerald-600', hrefBase: '/test',   moreLabel: 'Sabhi Tests',   moreHref: ROUTES.mockTests },
    blogs:     { icon: <Newspaper size={13} />, title: 'Blogs',       accent: 'text-purple-600',  hrefBase: '/blog',   moreLabel: 'Sabhi Blogs',   moreHref: ROUTES.blog },
};
    ...
    return (
        <section className="bg-white p-3 rounded-xl shadow-sm border border-slate-100">
            <h2 className={`text-[10px] md:text-[11px] font-black mb-2 flex items-center justify-between border-b border-slate-100 pb-1.5 uppercase tracking-wide ${meta.accent}`}>
                ...
                <Link to={toCanonicalUrl(meta.moreHref)} className="text-[9px] text-slate-400 hover:text-blue-600 flex items-center gap-0.5 normal-case font-bold">
                    {meta.moreLabel} <ArrowRight size={10} />
                </Link>
            </h2>
            <ul className="space-y-1.5" role="list">
                {items.map((item) => {
                    const linkPath = `${meta.hrefBase}/${item.slug}`;
                    // ✅ Link validation — broken links skip
                    if (!isValidInternalLink(linkPath)) return null;
                    return (
                        <li key={item.id}>
                            <Link to={toCanonicalUrl(linkPath)} className="group block">
        ...
        <div className="space-y-3">
            ...
            <section className="bg-white p-3 rounded-xl shadow-sm border border-slate-100">
                <h2 className="text-[10px] md:text-[11px] font-black mb-2 flex items-center justify-between border-b border-slate-100 pb-1.5 uppercase tracking-wide text-rose-600">
                    ...
                    <Link to={ROUTES.examCalendar} className="text-[9px] text-amber-600 hover:text-amber-700 normal-case font-black">
                        📅 Exam Calendar
                    </Link>
                </h2>
